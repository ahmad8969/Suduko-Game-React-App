    ...
    for(let hello=get_the_id_of_current_box; hello<=88; hello+=array_of_length){
      if(hello==0){
        ...
        const filter_two_arrays= (selectedId,completeArrayId)=> {
          ...
          for(let outerLoop=0; outerLoop<=complete_blocks_Array.length; outerLoop++) {
            for(let innerLoop=0; innerLoop<=complete_selection_array.length; innerLoop++) {
                if(complete_selection_array[innerLoop] ==  complete_blocks_Array[outerLoop]){
                  console.log('a');
                  return;
                }else{
                  console.log('b');
