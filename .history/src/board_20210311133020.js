...
function Board () {
  ...
  const Click = (idOfSpecificButton,i,j,click,e) =>{
    ...
    for(let hello=get_the_id_of_current_box; hello<=88; hello+=array_of_length){
      if(hello == 0){
        ...
        const gettingRandomNumber = () => {
          ...
          let character = String.fromCharCode(random);
          return character;
        }
    ...
    {nine_block_Element_into_Nine.map((s,i)=>{
      ...
      <NineBlock id={i}>
        {single_block_Into_Nine.map((s,j)=>
          ...
          <Block  rowId={i} value1={gettingRandomNumber()} click={Click} colId={j} keys={`${i}${j}`} isclick={isClicked}></Block>
