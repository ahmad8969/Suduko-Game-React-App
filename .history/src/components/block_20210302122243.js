...
const Block = (props) =>{
    ...
    let checkId = (e)=>{
        ...
        if(props.keyId == '0' || props.keyId == '3' || props.keyId == '6' ){
            let j= 0;
            let addition_of_three= 3+j;
            for( j; j<=6; addition_of_three){   

                for(let k=0; k<=first_Vertical_Block_Combination.length; k++){
                    let value = `${j}`+ first_Vertical_Block_Combination[k];
                    // var newSelection= document.getElementById(value)
                    // newSelection.style.color='yellow';
                    console.log(value);
                }
