...
function Board () {
  ...
  const Click = (idOfSpecificButton,i,j) =>{
    console.log(idOfSpecificButton.target.value)
    ...
    {nine_block_Element_into_Nine.map((s,i)=>{
      ...
      <NineBlock id={i}>
        {single_block_Into_Nine.map((s,j)=>
          random = Math.floor(Math.random() * (49 - 57)) + 57;
          console.log(random);
          <Block rowId={i} values={random} click={(e)=>{Click(`${i}${j}`,`${i}`,`${j}`)}} colId={j} keys={`${i}${j}`}></Block>
