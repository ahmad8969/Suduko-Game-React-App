...
function Board () {
  ...
  const XaxisSelectionLeft = (i,j)=>{
    for(let xAxis= j; xAxis>=0; xAxis--){
      let creatId= `${i}${xAxis}`;
      document.getElementById(creatId).style.backgroundColor="pink";
      xAxis_Selection_Array_Left.push(creatId);
    }
  }

  const YaxisSelectionDown = (i,j)=> {
    for(let xAxis1= j; xAxis1<=8; xAxis1++){
 ...
 const YaxisSelectionUp = (i,j)=> {
    for(let xAxis1= j; xAxis1>=0; xAxis1--){
      let creatId=  `${xAxis1}${i}`;
      document.getElementById(creatId).style.backgroundColor="pink";
    ...
    for(let hello=get_the_id_of_current_box; hello<=88; hello+=array_of_length){
      if(hello === 0){
        ...
        const filter_two_arrays= ()=> {
          const complete_selection_array = [
    ...
    {nine_block_Element_into_Nine.map((s,i)=>{
      ...
      <NineBlock id={i}>
        {single_block_Into_Nine.map((s,j)=>
          ...
          <Block  rowId={i} idsss={complete_selection_array}  click={Click} colId={j} keys={`${i}${j}`} isclick={isClicked}></Block>
