...
function Board () {
  ...
  const [getting_current_id, setGetting_current_id]=useState(null);
  ...
  const Click = (idOfSpecificButton,i,j,e) =>{
    let gettingId = setGetting_current_id(idOfSpecificButton);
    let I_id = setIid(i); 
    let J_id = setJid(j);
    alert(I_id,J_id,gettingId);
    // let e_id = setEvent(e);
    
    document.getElementById(gettingId).style.color="green"
    XaxisSelection(I_id);
    YaxisSelection(J_id);
    DiagonalSelectionUpward(gettingId);
    DiagonalSelectionDownward(gettingId);
    DiagonalSelectionDownwardLeftDown(gettingId);
    DiagonalSelectionUpwardLeftUp(gettingId);
