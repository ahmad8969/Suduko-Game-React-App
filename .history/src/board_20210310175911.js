  ...
  console.log(get_the_id_of_current_box); 
    for(let hello=get_the_id_of_current_box; hello<=88; hello+=array_of_length){
      if(hello==0){
        ...
        complete_selection_array.map(item =>
          {if(complete_blocks_Array.filter(item1 =>{item1 == item1})){
            console.log(`${item} found in 2`)
          }
        ...
        //  let test= complete_selection_array.map(item => { if( complete_blocks_Array.filter(item1 => {item1=== item}) ){ console.log(`${item} found in arr2`)} }) 
        //   return test;
