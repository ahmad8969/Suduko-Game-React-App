import React, { useState, memo } from "react"; //{useState}
import "./App.css";
...
function Board() {
  ...
  const [xaxisLeft,setXaxsisLeft]= useState([]);
  ...
  let puttingArrayInState = () => {
    setXaxsisLeft(...xAxis_Selection_Array_Left);
    console.log(xaxisLeft);
  }
                ...
                <div>
                  <NineBlock id={i}>
                    {single_block_Into_Nine.map((s, j) => (
                      <Block
                        rowId={i}
                        idsss={complete_selection_array}
                        click={Click}
                        colId={j}
                        keys={`${i}${j}`}
                        isclick={isClicked}
